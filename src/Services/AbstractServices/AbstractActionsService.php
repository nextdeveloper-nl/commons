 ...
 */
class AbstractActionsService
{
    public static function get(ActionsQueryFilter $filter = null, array $params = []) : Collection|LengthAwarePaginator
    {
        ...
        */
        if($filter == null) {
            $filter = new ActionsQueryFilter($request);
        }
     ...
     * @return mixed
     */
    public static function getByRef($ref) : ?Actions
    {
        return Actions::findByRef($ref);
    }
    ...
    {
        $object = Actions::where('uuid', $objectId)->first();

        $action = '\\NextDeveloper\\Commons\\Actions\\Actions\\' . Str::studly($action);
     ...
     * @return Actions|null
     */
    public static function getById($id) : ?Actions
    {
    ...
    {
        try {
            $obj = Actions::where('uuid', $uuid)->first();
