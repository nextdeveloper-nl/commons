...
class AbstractSocialMediaService
{
    public static function get(SocialMediaQueryFilter $filter = null, array $params = []) : Collection|LengthAwarePaginator
    {
        ...
        */
        if($filter == null) {
            $filter = new SocialMediaQueryFilter($request);
        }
    ...
    {
        $object = SocialMedia::where('uuid', $objectId)->first();

        $action = '\\NextDeveloper\\Commons\\Actions\\SocialMedia\\' . Str::studly($action);
