...
class AbstractExchangeRatesService
{
    public static function get(ExchangeRatesQueryFilter $filter = null, array $params = []) : Collection|LengthAwarePaginator
    {
        ...
        */
        if($filter == null) {
            $filter = new ExchangeRatesQueryFilter($request);
        }
    ...
    {
        $object = ExchangeRates::where('uuid', $objectId)->first();

        $action = '\\NextDeveloper\\Commons\\Actions\\ExchangeRates\\' . Str::studly($action);
